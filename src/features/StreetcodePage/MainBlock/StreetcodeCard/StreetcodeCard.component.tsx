...
const formatDate = (date?: Date): string =>
    fullMonthNumericYearDateFmtr.format(date).replace("р.", "року");
...
const StreetcodeCard = ({
    ...
    const {
        modalStore: { setModal, modalsState },
    } = useModalContext();
                    ...
                    setImagesForSlider(
                        imgs.filter(
                            (image) =>
                                ...
                                image.imageDetails?.alt ===
                                    ImageAssigment.animation.toString()
                        )
                    );
                    streecodePageLoaderContext.addBlockFetched();
                })
                .catch((e) => {});
            TransactionLinksApi.getByStreetcodeId(id).then((x) =>
                setArlink(x.url)
            );
        }
    }, [streetcode]);

    return streecodePageLoaderContext.isPageLoaded ? (
        <div className="card">
                        ...
                        {imagesForSlider.map((image, index) => {
                                ...
                                return (
                                    <div>
                                        <img
                                            ...
                                            alt={
                                                imagesForSlider[0].imageDetails
                                                    ?.alt
                                            }
                                        />
                                        <img
                                            ...
                                            style={{ objectFit: "contain" }}
                                            alt={
                                                imagesForSlider[1].imageDetails
                                                    ?.alt
                                            }
                                        />
                                    </div>
                            ...
                            return (
                                <img
                                    ...
                                    style={{ objectFit: "contain" }}
                                    alt={
                                        imagesForSlider[index].imageDetails?.alt
                                    }
                                />
                            );
                        })}
            ...
            <div className="rightSider">
                <div className="streetcodeIndex">
                    History-код #
                    {streetcode?.index ?? 0 <= 9999
                        ? `000${streetcode?.index}`.slice(-4)
                        : streetcode?.index}
                </div>
                <h2 className="streetcodeTitle">{streetcode?.title}</h2>
                <div className="streetcodeDate">{streetcode?.dateString}</div>
                <div className="tagListWrapper">
                    <TagList
                        tags={streetcode?.tags.filter(
                            (tag: StreetcodeTag) => tag.isVisible
                        )}
                ...
                <p className="teaserBlock">{streetcode?.teaser}</p>
                ...
                <div className="cardFooter">
                    {audio?.base64 && audioIsLoaded ? (
                        ...
                        <Button disabled type="primary" className="audioBtn">
