...
function FromDateToString(date: Date, dataViewType: DateViewPattern) {
    ...
    case DateViewPattern.MonthYear:
        return format(date, 'yyyy, MMMM', { locale: uk });
    case DateViewPattern.SeasonYear:
        return `${format(date, 'yyyy,')} ${getSeason(dayjs(date))}`;
    case DateViewPattern.DateMonthYear:
        return format(new Date(date), 'yyyy, d MMMM', { locale: uk });
    default:
        return '';
    }
}
