...
const MainBlockAdmin = React.memo(
    ({
        ...
        const [popoverProps, setPopoverProps] = useState<TagPreviewProps>(
            tagPreviewPropsList[0]
        );
        ...
        const getErrorMessage = (maxLength: number = maxTagLength) =>
            `Довжина не повинна перевищувати ${maxLength} символів`;
        ...
        const onSelectTag = (selectedValue: string) => {
            const deletedTag = tagsStore.getTagToDeleteArray.find(
                (tag) => tag.title === selectedValue
            );
            if (deletedTag) {
        ...
        const onDeselectTag = (deselectedValue: string) => {
            const tag = selectedTags.find(
                (t) => t.title === deselectedValue
            ) as StreetcodeTagUpdate;
            if (tag?.isPersisted) {
            ...
            setSelectedTags(
                selectedTags.filter((t) => t.title !== deselectedValue)
            );
        };
        ...
        return (
            <div className="mainblock-add-form">
                    ...
                    rules={[
                        {
                            ...
                            max: 9999,
                            message:
                                "Введіть номер history-коду від 1 до 9999, будь ласка",
                        },
                        {
                            validator: async (_, value) => {
                                if (value && /^\d+$/.test(value)) {
                                    if (Id) {
                                        const streetcode =
                                            await StreetcodesApi.getById(Id);
                                    ...
                                    return Promise.reject(
                                        new Error("Цей номер вже зайнятий")
                                    );
                                }
                            },
                        },
                ...
                <div className="display-flex-row p-margin">
                    <p className={switchState ? "grey-text" : "red-text"}>
                        Постать
                    </p>
                ...
                <Form.Item
                    ...
                    rules={[
                        {
                            required: true,
                            message: "Введіть назву history-коду, будь ласка",
                        },
                        {
                            max: 100,
                            message:
                ...
                {streetcodeType.current === StreetcodeType.Person ? (
                    <Space.Compact className="display-flex-column">
                        <Form.Item
                            label="Ім'я"
                            name="name"
                ...
                <Form.Item
                    ...
                    rules={[
                        {
                            required: true,
                            message: "Введіть транслітерацію",
                            max: 100,
                        },
                        {
                            pattern: /^[a-z0-9-]+$/,
                                    ...
                                    <Popover
                                        ...
                                        content={
                                            <p className="label-tags-block-info-container-content">
                                                При обиранні теги є невидимими
                                                для користувача (фон тегу
                                                сірий), тобто він не
                                                відображається на головній
                                                картці history-коду. Якщо
                                                натиснути на тег, його стан
                                                зміниться на видимий (фон -
                                                білий). Нижче є розширення
                                                наводячи на які, можна побачити,
                                                які теги будуть вміщатись на
                                                головній картці history-коду.{" "}
                                            </p>
                                        }
                                    >
                ...
                <div className="teaser-form-item">
                    <Form.Item
                        ...
                        rules={[
                            {
                                required: true,
                                message: "Введіть тизер",
                                max: teaserMaxCharCount,
                            },
                        ]}
                    >
                        <Input.TextArea
