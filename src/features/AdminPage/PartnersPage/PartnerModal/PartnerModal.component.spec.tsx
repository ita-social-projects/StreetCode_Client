...
jest.mock("@stores/root-store", () => ({
    ...
    default: () => ({
        partnersStore: {
            ...
            createPartner: (partner: PartnerCreateUpdate) =>
                PartnersApi.create(partner),
            updatePartner: (partner: PartnerCreateUpdate) =>
...
jest.mock("@/features/AdminPage/PartnersPage/PartnerLink.component", () => {
    return {
        ...
        default: (a: any) => {
            return (
                <div role="listitem" aria-label="partnerLink">
                    {JSON.stringify(a)}
                </div>
            );
        },
...
describe("PartnerModal", () => {
    ...
    test("creating partner only with required fields", async () => {
        (PartnersApi.create as jest.Mock).mockResolvedValueOnce({
            id: 1,
        } as Partner);
    ...
    test("creating partner with all possible fields", async () => {
        ...
        const streetcodeSelect = screen.getByRole("combobox", {
            name: /History-коди:/,
        });
        ...
        await waitFor(() => {
            ...
            userEvent.click(
                screen.getByRole("button", { name: /Streetcode 1/i })
            );
    ...
    test("submiting social link", async () => {
        const dom = render(
            <PartnerModal open={true} setIsModalOpen={() => {}} />
        );

        userEvent.click(
            screen.getByRole("button", { name: /Додати соціальну мережу/ })
        );
        const form = dom.baseElement
        ...
        await waitFor(() => {
            expect(dom.baseElement.innerHTML).toContain(
