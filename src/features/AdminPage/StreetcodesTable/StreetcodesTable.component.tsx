...
import { Button, Dropdown, Empty, MenuProps, Pagination, Space } from "antd";
...
const StreetcodesTable = () => {
    ...
    const [requestGetAll, setRequestGetAll] =
        useState<GetAllStreetcodesRequest>(requestDefault);

    const setRequest = () => {
        let page = pageRequest;
        const filter =
            statusRequest.length === 0 ? null : `Status:${statusRequest}`;
        if (
            requestGetAll.Title !== titleRequest ||
            requestGetAll.Filter !== filter
        ) {
            setPageRequest(1);
            setCurrentPages(1);
            page = 1;
    ...
    const columnsNames = [
        {
            ...
            onCell: (record: MapedStreetCode) => ({
                onClick: () =>
                    window.open(
                        `${FRONTEND_ROUTES.ADMIN.BASE}/${record.url}`,
                        "_blank"
                    ),
            }),
        },
            ...
            render: (_: unknown, record: MapedStreetCode) => (
                <>
                    {record.status !== "Видалений" ? (
                        <>
                            <Link
                                to={`${FRONTEND_ROUTES.ADMIN.EDIT_STREETCODE}/${record.key}`}
                            >
                                <EditOutlined
                            ...
                            />
                            <Link
                                to={`${FRONTEND_ROUTES.ADMIN.ANALYTICS}/${record.key}`}
                            >
                                <BarChartOutlined className="actionButton" />
                            </Link>
                        </>
                    ) : (
                        <RollbackOutlined
    ...
    const fetchPaginatedData = async () => {
        ...
        const getAllStreetcodesResponse = await StreetcodesApi.getAll(
            requestGetAll
        );
