...
import {
    ...
    prevArticleClickEvent,
    toArticleRedirectClickEvent,
    ...
    useEffect(() => {
        NewsApi.getNewsAndLinksByUrl(newsUrl)
            .then((res) => {
                ...
                setParCount(
                    ((value.news.text as string)?.match(/<p\b[^>]*>/gi) || [])
                        .length
                );
            })
            .catch(() => {
        ...
        if (newsValue) {
            if (newsValue.news.imageId != null) {
                ...
                if (newsValue.news.image) {
                    const imgUrl = base64ToUrl(
                        getImage(newsValue.news.imageId!)?.base64,
                        getImage(newsValue.news.imageId!)?.mimeType
                    );
                    newsimg.src = imgUrl!;
    ...
    return (
        <>
            <Helmet>
                ...
                <meta
                    name="description"
                    content="Будьте в курсі останніх новин від «Historycode»."
                />
            </Helmet>
            newsValue ? (
                ...
                <div className="wrapper" ref={wrapperRef}>
                    ...
                    <div className="NewsHeader">
                        ...
                        <h3 className="news-date">
                            {dayjs(newsValue?.news.creationDate).format(
                                "DD.MM.YYYY"
                            )}
                        </h3>
                    </div>
                    ...
                    <div className="newsLinks">
                        <Link
                            className={`Link ${
                                newsValue?.prevNewsUrl === null ? "toHide" : ""
                            }`}
                        ...
                        <Link
                            className={`Link ${
                                newsValue?.nextNewsUrl === null ? "toHide" : ""
                            }`}
