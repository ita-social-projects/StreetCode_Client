...
const SubtitlesApi = {
    ...
    getByStreetcodeId: (streetcodeId: number) => Agent.get<Subtitle>(
        `${API_ROUTES.SUBTITLES.GET_BY_STREETCODE_ID}/${streetcodeId}`,
    ),

    create: (subtitle: Subtitle) => Agent.post<Subtitle>(
        `${API_ROUTES.SUBTITLES.CREATE}`,
        subtitle,
    ),

    update: (subtitle: Subtitle) => Agent.put<Subtitle>(
        `${API_ROUTES.SUBTITLES.UPDATE}`,
        subtitle,
    ),
